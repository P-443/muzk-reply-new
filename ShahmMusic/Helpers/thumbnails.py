...
async def gen_thumb(videoid, user_id):
    ...
    try:
        ...
        # 1. تصميم الغلاف المربع على اليسار بالإطار الأبيض
        sq_size = 400
        ...
        # دمج صورة العضو والدائرة الأصلية كشعار
        background.paste(x, (90 + border_size - 60, 150 + border_size - 60), mask=x)

        draw = ImageDraw.Draw(background)
        font_title = ImageFont.truetype("ShahmMusic/Helpers/utils/font2.ttf", 45)
        ...
        # 2. كتابة العناوين جهة اليمين
        x_text = 560
        y_text = 160

        draw.text((x_text, y_text), "STARTED PLAYING", fill="#00E5FF", font=arial)
        
        para = textwrap.wrap(title, width=22)
        if len(para) > 0 and para[0]:
            draw.text((x_text, y_text + 45), para[0], fill="white", font=font_title)
        if len(para) > 1 and para[1]:
            draw.text((x_text, y_text + 100), para[1], fill="white", font=font_title)

        draw.text((x_text, y_text + 175), "Telegram Files", fill="#CCCCCC", font=font_sub)
        draw.text((x_text, y_text + 220), "Shahm Music", fill="#CCCCCC", font=font_sub)
...
async def gen_qthumb(videoid, user_id):
    ...
    try:
        ...
        draw.text((x_text, y_text), "ADDED TO QUEUE", fill="#FF9800", font=arial)
